...
public class ChatroomService extends AbstractLinkService implements IChatroomService {
    ...
    public List<Chatroom> pageRoom(String principal, int limit, long offset) {
        ICube cube = cube(principal);
        String cjql = String.format("select {'tuple':'*'}.limit(%s).skip(%s) from tuple chatrooms %s where {'tuple.creator':'%s'}", limit, offset, Chatroom.class.getName(), principal);
        IQuery<Chatroom> query = cube.createQuery(cjql);
    ...
    @Override
    public List<RoomMember> pageAnyRoomMember(String principal,String room, int limit, long offset) {
        ICube cube = cube(principal);
